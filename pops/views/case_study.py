...
class ApprovedAndUserCaseStudyListView(TemplateView):
    ...
    def get_queryset(self):
        return CaseStudy.objects.prefetch_related('host_set','pest_set__pest_information').filter(Q(staff_approved = True ) | Q(created_by = self.request.user)).order_by('-date_created')

    def get_context_data(self, **kwargs):
            # Call the base implementation first to get the context
            context = super(ApprovedAndUserCaseStudyListView, self).get_context_data(**kwargs)
            case_studies=self.get_queryset()
            context['staff_approved_case_studies'] = case_studies.filter(staff_approved = True)
